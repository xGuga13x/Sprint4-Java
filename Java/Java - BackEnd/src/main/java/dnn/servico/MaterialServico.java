...
public class MaterialServico {
    ...
    public Material buscar(int id) throws SQLException {
        try (Connection c = Conexao.conectar();
             PreparedStatement s = c.prepareStatement(
                     "SELECT id_material,nome,descricao,quantidade,quantidade_minima,unidade," +
        ...
        try (Connection conn = Conexao.conectar()) {
            ...
            try {
                ...
                try (PreparedStatement si = conn.prepareStatement(
                        "SELECT NVL(MAX(id_material),0)+1 FROM tdb_Material");
                     ResultSet ri = si.executeQuery()) {
                    id = ri.next() ? ri.getInt(1) : 1;
                }
                try (PreparedStatement s = conn.prepareStatement(
                        "INSERT INTO tdb_Material(id_material,nome,descricao,quantidade," +
                                "quantidade_minima,unidade,validade) VALUES(?,?,?,?,?,?,TO_DATE(?,'YYYY-MM-DD'))")) {
                    s.setInt(1, id);
    ...
    public void atualizar(int id, Material m) throws SQLException {
        ...
        try (Connection c = Conexao.conectar();
             PreparedStatement s = c.prepareStatement(
                     "UPDATE tdb_Material SET nome=?,descricao=?,quantidade=?," +
                             "quantidade_minima=?,unidade=?,validade=TO_DATE(?,'YYYY-MM-DD') WHERE id_material=?")) {
    ...
    public void excluir(int id) throws SQLException {
        try (Connection c = Conexao.conectar();
             PreparedStatement s = c.prepareStatement("DELETE FROM tdb_Material WHERE id_material=?")) {
            s.setInt(1, id);
            if (s.executeUpdate() == 0) throw new ApiExcecao("Material nao encontrado.", 404);
        }
    }
